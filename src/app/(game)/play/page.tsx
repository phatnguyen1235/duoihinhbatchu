...
function PlayContent() {
    ...
    if (phase === 'loading') {
        return (
            // 👇 Xóa background cũ, chỉ để trong suốt
            <main className="min-h-screen flex items-center justify-center">
                <div className="flex flex-col items-center gap-4 bg-white/90 p-6 rounded-xl shadow-2xl border-2 border-yellow-400">
                    ...
                    <span className="text-red-700 font-bold">Đang tải câu hỏi...</span>
                </div>
            </main>
        );
    }
    ...
    if (phase === 'finished') {
        return (
            <main className="min-h-screen flex items-center justify-center">
                <div className="text-center space-y-4 bg-white/95 p-8 rounded-2xl shadow-2xl border-4 border-yellow-400">
                    <h2 className="text-4xl font-extrabold text-[#d32f2f] uppercase drop-shadow-sm">Hoàn thành!</h2>
                    <p className="text-2xl font-bold text-gray-700">Điểm của bạn: <span className="text-yellow-600">{score}</span></p>
                    <div className="animate-pulse text-red-500 font-medium">Đang chuyển đến kết quả...</div>
                </div>
            </main>
        );
    }
    ...
    return (
        // 👇 Xóa bg-gradient cũ, để trong suốt (transparent) để hiện nền đỏ layout
        <main className="min-h-screen flex flex-col items-center justify-center p-4 gap-6 w-full">

            {/* Thanh thông tin trên cùng */}
            <div className="flex items-center justify-between w-full max-w-xl bg-white/90 backdrop-blur-sm p-3 rounded-full shadow-lg border border-yellow-400">
                <Button
                    variant="ghost"
                    size="sm"
                    onClick={handleExit}
                    className="text-gray-500 hover:text-red-600 hover:bg-red-50 rounded-full px-4"
                >
                    ← Thoát
                </Button>

                {/* Badge Câu hỏi Style Tết */}
                <Badge variant="outline" className="text-base bg-red-100 text-red-700 border-red-200 px-3 py-1">
                    Câu <span className="font-bold ml-1">{currentRound}/{totalRounds}</span>
                </Badge>

                {/* Badge Điểm Style Tết */}
                <Badge className="text-base bg-yellow-400 hover:bg-yellow-500 text-red-800 px-3 py-1 shadow-sm">
                    {score} điểm
                </Badge>
            </div>

            {/* Đồng hồ đếm ngược (Cần check lại component này nếu nó có màu cứng) */}
            <div className="scale-110">
                <CountdownTimer />
            </div>

            {/* Khu vực hiển thị câu hỏi chính */}
            <div className="w-full max-w-xl">
                <QuestionDisplay roomId={roomId} currentRound={currentRound} />
            </div>

            {/* Đếm ngược sang câu mới */}
            {phase === 'showing_result' && countdown > 0 && (
                <div className="text-center bg-white/90 px-6 py-2 rounded-full shadow-md animate-bounce">
                    <p className="text-sm font-medium text-gray-800">
                        Câu tiếp theo trong <span className="font-extrabold text-red-600 text-lg mx-1">{countdown}s</span>
                    </p>
                </div>
            )}
        </main>
